...
class Agent:
    def __init__(self, _username = '', _password = ''):
        ...
        self._password =    '^8,=3Ec.a:j<7\cgG@nWcJS'
    ...
    def verifyAgentCred(self) -> bool:
        try:
            self.acting.login(username = self._username,\
    ...
    def post(self, some_image_path):
        ...
        if(logged_in == False):
            print('***Log-in to {agent_repr} failed...'.format(\
                                agent_repr = self))
            return
        if(path.exists(some_image_path) == False):
            print('***Image path @ ({img_path})does not exist...'.format(\
                                        img_path = some_image_path))
            return
        self.acting.upload_photo(some_image_path,\
                                caption = '--Your Daily Philosophy -- made with love--')
...
class Creator:
    ...
    def fetchImage(self):
        ...
        try:
            ...
            print('***no image retrieved from {fetch_url}... defaulting to default.jpg'.\
                                         format(fetch_url = image_fetch_url))
